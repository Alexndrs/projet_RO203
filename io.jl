...
function readInputFile(inputFile::String)::Matrix{Int}
    ...
    # Initialise une liste vide pour stocker les données lues
    puzzleList = []

    # Pour chaque ligne du fichier d'entrée
    for line in data
        # Divise la ligne en utilisant la virgule comme délimiteur,
        # convertit chaque élément en entier et les stocker dans un tableau
        row = parse.(Int, split(line, ","))
        push!(puzzleList, row)
    end

    puzzle = mapreduce(permutedims, vcat, puzzleList)

    # Retourne le puzzle lu sous forme de matrice
    return puzzle
end
...
function displayGrid(G::Matrix{Int})
    ...
    println("       |", "-"^(2*max_width), "+", join(["-"^(2*max_width)*"+" for i in 1:(m-1)]))

    # Affiche chaque ligne du tableau
    for i in 1:n
        println("       |", lpad(string(G[i,1]), max_width, " ")," "^(max_width), "|", join([lpad(string(G[i,j]), max_width, " ")*" "^(max_width) * "|" for j in 2:m]))
...
function displaySolution(G::Matrix{Int}, X)
    ...
    # Affiche chaque ligne du tableau
    for i in 1:n
        println("       |", caseString(i,1,max_width,G,X)," "^(max_width), "|", join([caseString(i,j,max_width,G,X)*" "^(max_width) * "|" for j in 2:m]))
